...
## モバイルカレンダー
### 達成したこと
カレンダーの予定から往訪先を推定するモデルの教師データを登録するための環境を実装した。
その中で、ユーザの投稿した教師データを登録するWebAPIをElixirで実装した。

### 達成できなかったこと
SPEEDAモバイルの開発が中断されたため、
集めた教師データでモデルの学習、推定にかかわるプログラムを実装しなかった。

## トレンドとニュースの紐付け
### 達成したこと
モデルを訓練するプログラムをK8S環境で動かすパイプライン、および、モデルによる推定を提供するWeb APIを実装した。

### 達成できなかったこと
トレンドとニュースの紐付けの予測性能を原稿のクエリによる紐付け以上にできず、モデルをリリースできなかった。
Weak supervisionをはじめとする技術調査と試行で作業が止まっている。
...
### 達成できなかったこと
SecretsがBlue/Greenで共有されている、Dockerイメージに開発時にしか必要のないリソースが含まれているなどの運用・保守や処理性能の問題に課題がああり、必要な英語のニュースカテゴライザをリリースできていない。
...
## 今期パフォーマンス総評
Falconの移行作業で、インフラと機械学習、アプリケーションの3領域にまたがる問題を釘村さんと一緒に対処し、
ニュースカテゴライザをリリースした。作業のなかで、apacheの設定の変更、k8sのノード増強などSREとPDTのペアプログラミングでもインフラメンバーだけで担当する領域も一緒に作業でき、勉強になった。

個人OKRについては、機械学習技術の試用、Kubeflowによる運用に積み残しがある。
トレンドとニュースの紐付けでは、以下の調査した手法のうち、弱教師あり学習と不均衡データにまつわる手法を試したが、本番へのリリースにつなげることはできていない。また、今期新しいモデルをリリースしていいため、Kubeflowをためす機会の逃してしまった。
https://portal.uzabase.com/speeda/tech/ds/articles

### 今期の自己タイトル
M5-3
前期までに実装したニュースカテゴライザをリリースし、
企業や業界のモデルを作成するパイプラインを前期に構築した経験をいかし、
トレンドとニュースの紐付けモデルのパイプラインを構築した。
また、技術調査で調査した論文の数は前期と比べて2倍になった。
以上をふまえて、前期よりも低いパフォーマンスではない考えている。
